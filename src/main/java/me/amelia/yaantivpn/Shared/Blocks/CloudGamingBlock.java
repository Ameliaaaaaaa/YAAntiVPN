...
public class CloudGamingBlock {
    private final JSONObject ipData;

    private final BlockProvider blockProvider;

    public CloudGamingBlock(JSONObject ipData, BungeeConfig bungeeConfig) {
        this.ipData = ipData;
        this.blockProvider = bungeeConfig.getBlocks().get("CLOUD_GAMING");
    }

    public CloudGamingBlock(JSONObject ipData, BukkitConfig bukkitConfig) {
        this.ipData = ipData;
        this.blockProvider = bukkitConfig.getBlocks().get("CLOUD_GAMING");
    }

    public boolean blockAddress() {
        return (this.blockProvider.isEnabled() && this.ipData.getJSONArray("types").toList().contains(Integer.valueOf(7)));
    }
