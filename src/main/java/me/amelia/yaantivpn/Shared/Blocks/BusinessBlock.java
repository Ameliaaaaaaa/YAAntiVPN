...
public class BusinessBlock {
    ...
    public BusinessBlock(JSONObject ipData, BungeeConfig bungeeConfig) {
        this.ipData = ipData;
        this.blockProvider = bungeeConfig.getBlocks().get("BUSINESS");
    }

    public BusinessBlock(JSONObject ipData, BukkitConfig bukkitConfig) {
        this.ipData = ipData;
        this.blockProvider = bukkitConfig.getBlocks().get("BUSINESS");
    }

    public boolean blockAddress() {
        return (this.blockProvider.isEnabled() && this.ipData.getJSONArray("types").toList().contains(Integer.valueOf(2)));
    }
